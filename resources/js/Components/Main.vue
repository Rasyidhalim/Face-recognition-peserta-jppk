      ...
      <div class="relative w-full max-w-sm bg-black rounded-3xl overflow-hidden border-2 border-emerald-800 shadow-2xl">
        ...
        <div v-for="(box, index) in results?.data" :key="index"
          ...
          <div :style="{
            ...
            <span>{{ box.label === 'TIDAK DIKENAL' ? 'TIDAK DIKENAL' : (!box.is_lively ? 'KEDIPKAN MATA!' : 'DIKENALI') }}</span>
            <span style="font-size: 11px; font-weight: normal; margin-top: 2px;">Acc: {{ box.confidence ? (box.confidence * 100).toFixed(1) : 0 }}% | Thr: 70%</span>
          </div>
        </div>
